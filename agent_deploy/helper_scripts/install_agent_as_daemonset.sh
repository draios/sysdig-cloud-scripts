...
# Installer for Sysdig Agent as Daemonset in Kubernetes

# TODO enable strict bash settings
#set -euo pipefail
set -e
...
function download_yamls {
    ...
    curl -s -o /tmp/sysdig-agent-clusterrole.yaml \
        https://raw.githubusercontent.com/draios/sysdig-cloud-scripts/master/agent_deploy/kubernetes/sysdig-agent-clusterrole.yaml
    echo "* Downloading Sysdig config map yaml"
    curl -s -o /tmp/sysdig-agent-configmap.yaml \
    ...
    curl -s -o /tmp/sysdig-agent-daemonset-v2.yaml \
...
function install_kubernetes_agent {
